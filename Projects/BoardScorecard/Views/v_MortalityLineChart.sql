...
/****** Object:  View [BoardScorecard].[v_MortalityLineChart]    Script Date: 9/19/2023 9:24:18 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [BoardScorecard].[v_MortalityLineChart] AS
...
from(
	...
	select
		...
		,maxdate as Datejoin
		,case 
			when Date = DATEADD(month,-3,maxdate) then 1 when Date = DATEADD(month,-4,maxdate) then 2 
			when Date = DATEADD(month,-5,maxdate) then 3 when Date = DATEADD(month,-6,maxdate) then 4
			when Date = DATEADD(month,-7,maxdate) then 5 when Date = DATEADD(month,-8,maxdate) then 6
			when Date = DATEADD(month,-9,maxdate) then 7 when Date = DATEADD(month,-10,maxdate) then 8
			when Date = DATEADD(month,-11,maxdate) then 9 when Date = DATEADD(month,-12,maxdate) then 10
			when Date = DATEADD(month,-13,maxdate) then 11 when Date = DATEADD(month,-14,maxdate) then 12
			end as sortorder
	from(
		select 
			d.StageID
